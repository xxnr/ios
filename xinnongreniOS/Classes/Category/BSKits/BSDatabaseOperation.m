...
- (NSString *)description {
    return [NSString stringWithFormat:@"id=%@, brandName=%@, value=%@, timeStamp=%@", _itemId,_brandName, _itemObject, _createdTime];
}
...
static NSString *const UPDATE_ITEM_SQL = @"REPLACE INTO %@ (id, brandName, json, createdTime) values (?, ?, ?, ?)";

static NSString *const QUERY_ITEM_SQL = @"SELECT json, createdTime from %@ where id = ? Limit 1";

static NSString *const QUERY_Brand_ITEM_SQL = @"SELECT json, createdTime from %@ where brandName = ?";
...
- (BOOL)putObject:(id)object withId:(NSString *)objectId intoTable:(NSString *)tableName {
    ...
    __block BOOL result;
    [_dbQueue inDatabase:^(FMDatabase *db) {
        result = [db executeUpdate:sql, objectId, brandName, jsonString, createdTime];
    }];
    if (!result) {
...
- (BSDatabaseOperationItem *)getBSDatabaseOperationItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if ([rs next]) {
            ...
            brandName = [rs stringForColumn:@"brandName"];
        }
        [rs close];
    }];
    if (json) {
...
- (NSArray *)getAllTheBrandIdGoodsWith:(NSString *)brandName fromTable:(NSString *)tableName {
    NSMutableArray *allBrandGoods = [NSMutableArray array];
    
    if ([BSDatabaseOperation checkTableName:tableName] == NO) {
        return nil;
    }
    NSString * sql = [NSString stringWithFormat:QUERY_Brand_ITEM_SQL, tableName];
    __block NSString * json = nil;
    __block NSDate * createdTime = nil;
    [_dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet * rs = [db executeQuery:sql, brandName];
        while ([rs next]) {
            json = [rs stringForColumn:@"json"];
            createdTime = [rs dateForColumn:@"createdTime"];
            if (json) {
                ...
                item.itemObject = result;
                item.createdTime = createdTime;
                [allBrandGoods addObject:item.itemObject];
            }
        }
        [rs close];
    }];

    return allBrandGoods;
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    [_dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([rs next]) {
            ...
            item.brandName = [rs stringForColumn:@"brandName"];
