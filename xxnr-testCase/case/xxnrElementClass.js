...
xxnrElementClass.xxnrClientRegister = function (window) {
    return{
    totalNum:function () {
        return window.staticTexts()[8].value();
    },
    todayNum:function () {
        return window.staticTexts()[9].value();
    },
    cell:function (index) {
        return window.tableViews()[1].cells()[index];
    },
...
xxnrElementClass.selPro = function (window) {
    ...
    groupBtn:function(i){
        return window.tableViews()[0].groups()[i].buttons()[0];
    },

    groupName:function(i){
           return window.tableViews()[0].groups()[i].staticTexts()[0];
        },
    groupSel:function(i){
           return window.tableViews()[0].groups()[i].staticTexts()[1];
        },
    cells:function(i){
        return window.tableViews()[0].cells();
    },
     cell:function(j){
        return window.tableViews()[0].cells()[j];
    },
     makeSure:function () {
        return window.buttons()["确定"];
     }
