...
function Ab() {
    ...
    test("未支付订单--用户订单",function () {
        ...
        delay(1);
        window.logElementTree();
        logMessage("'"+xxnrElementClass.myorder(window).userOrderStaticText(0)+"'");
    ...
    test("未支付订单--县级订单",function () {
        ...
        var orderStr = xxnrElementClass.rscorder(window).userOrderId(0);
        var orderId = orderStr.substr(5,orderStr.length - 5);
        xxnrElementClass.RSCOrder(window).holdPayTab().tap();
        judge(orderId,"待付款");
    ...
    test("用户支付部分定金--县级订单",function () {
        ...
        xxnrElementClass.mine(window).rscOrderBtn().tap();
        delay(1);
        assertEquals("待付款",xxnrElementClass.rscorder(window).userOrderStaticText(0));
    ...
    test("用户线下支付剩余定金--县级订单",function () {
        ...
        assertEquals("付款待审核",xxnrElementClass.rscorder(window).userOrderStaticText(0));
    ...
    test("用户线下支付剩余尾款--县级订单",function () {
        ...
        xxnrElementClass.RSCOrder(window).totalTab().tap();
        delay(1);
        assertEquals("待自提",xxnrElementClass.rscorder(window).userOrderStaticText(0));
    
    
        var orderStr = xxnrElementClass.rscorder(window).userOrderId(0);
        var orderId = orderStr.substr(5,orderStr.length - 5);
        // target.frontMostApp().windows()[0].buttons()["待自提"].tap();
        xxnrElementClass.RSCOrder(window).carryTab().tap();
        judge(orderId,"待自提");
    ...
    test("去自提二三商品(已完成)--县级订单",function () {
        ...
        delay(1);
        xxnrElementClass.RSCOrder(window).totalTab().tap();
        assertEquals("已完成",xxnrElementClass.rscorder(window).userOrderStaticText(0));
